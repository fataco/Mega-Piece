...
__new = { playerMoney = nil, CURRENCY = nil, YOUR_MONEY = tr('Your Points') .. ':' }

ProtocolGame.registerExtendedOpcode(GameServerOpcodes.NpcTrade, 
function (protocol, opcode, buffer)
   loadstring("__newBuffer = "..buffer)()
   if __newBuffer then
      __new.playerMoney = __newBuffer.qnt
      __new.CURRENCY = __newBuffer.name  
      if yourMoneyLabel then
         yourMoneyLabel:setText(__new.YOUR_MONEY) 
      end
   end
end)
...
function init()
  ...
  yourMoneyLabel = setupPanel:getChildById('yourMoney') 
...
function terminate() 
  ProtocolGame.unregisterExtendedOpcode(GameServerOpcodes.NpcTrade)
  initialized = false
  npcWindow:destroy()
...
function refreshPlayerGoods()
  ...
  moneyLabel:setText(formatCurrency(((__new.playerMoney ~= nil) and __new.playerMoney or playerMoney)))
...
function onCloseNpcTrade()
  __new = { playerMoney = nil, CURRENCY = nil, YOUR_MONEY = __new.YOUR_MONEY }
  if yourMoneyLabel then
     yourMoneyLabel:setText(YOUR_MONEY)
  end
  hide()   
end

function onPlayerGoods(money, items)
  playerMoney = (__new.playerMoney ~= nil) and __new.playerMoney or money
  playerItems = {}
  for key,item in pairs(items) do
...
function formatCurrency(amount)
  if CURRENCY_DECIMAL then
    ...
    return amount .. ' ' .. ((__new.CURRENCY == nil) and CURRENCY or __new.CURRENCY)
  end
end
