...
-- private functions
local function addButton(id, description, icon, callback, panel, toggle, front, seta)
  local class
  if toggle then
  ...
  if not button then
    ...
    if seta then
      g_ui.createWidget('SetaPraCima', button) 
    end
    if front then
...
function addLeftGameButton(id, description, icon, callback, front, seta)     
  if seta then       
     local node = g_settings.getNode('firsttimeseta') 
     if node then         
        if node[id] then
           seta = false
        end
  ...
  return addButton(id, description, icon, callback, leftGameButtonsPanel, false, front, seta)
end
...
function addRightGameToggleButton(id, description, icon, callback, front, seta)   
  if seta then       
     local node = g_settings.getNode('firsttimeseta') 
     if node then         
        if node[id] then
  ...
  return addButton(id, description, icon, callback, rightGameButtonsPanel, true, front, seta)
end
