...
function init()
  ...
  ProtocolGame.registerExtendedOpcode(GameServerOpcodes.SkillInformations, function(protocol, opcode, buffer)
      --print(buffer)
      loadstring('__buffer = ' .. buffer)() -- transforma texto em tabela lua 
      if __buffer and __buffer.skill then
          for _, skill in pairs(__buffer.skill) do
            --print('>>', skill[1], skill[2], skill[3])
            setSkillValue('skill_new_' .. skill[1], skill[2])
            setSkillPercent('skill_new_' .. skill[1], skill[3], tr('You have %s percent to go', 100 - skill[3]))
          end
      end            
  end)
...
function refresh()
  ...
  skillsWindow:setContentMaximumHeight(390 + 90)
end
